...
st.write("This app generates a timetable for summer courses based on faculty and course data from Google Sheets.")
...
if st.button("Generate Timetable"):
    try:
        ...
        df_from_google_sheet = pd.read_csv(csv_export_url)
        csv_file = "/tmp/courses.csv"
        df_from_google_sheet.to_csv(csv_file, index=False)

        summer2026_courses = {batch: [] for batch in batches}
            ...
            for row in reader:
                ...
                for column_header, cell_value in row.items():
                    ...
                    if match:
                        ...
                        if extracted_batch in batches and "Summer2026" in cell_value:
                            summer2026_courses[extracted_batch].append(f"{course_code} — {course_name}")
        ...
        # Combine data
        summer_courses_list = []
        for batch, courses in summer2026_courses.items():
