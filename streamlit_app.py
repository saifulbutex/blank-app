...
try:
    ...
    # Extract semesters (optimized - scan only A1:P50 range)
    semesters = set()
    df_subset = df_courses.iloc[:50, :16]  # First 50 rows, columns A-P
    ...
    # Get only the last 3 semesters
    last_3_semesters = all_semesters[:3]
    semester_options = ["Select a semester"] + last_3_semesters
...
if st.button("Generate Timetable"):
    if selected_semester == "Select a semester":
        st.error("Please select a semester before generating the timetable.")
    else:
        try:
            # Process courses
            sheet_id = google_sheet_url_courses.split('/d/')[1].split('/edit')[0]
